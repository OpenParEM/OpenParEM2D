                 ...
                 double **alphaList, double **betaList, PetscInt EtSize, PetscInt EzSize, PetscMPIInt rank)
{
   ...
   PetscPrintf(PETSC_COMM_WORLD,"         post processing ...\n");
   ...
   if (nullEigenvalueCount > 0) {ierr=PetscPrintf (PETSC_COMM_WORLD,"         null space eigenvalue count: %d\n",nullEigenvalueCount); CHKERRQ(ierr);}
   ...
   *betaList=(double *)malloc (projData->solution_active_mode_count*sizeof(double));
   ...
   // clean up
   if (Efield) {ierr=VecDestroy(&Efield); CHKERRQ(ierr);}
   if (Hfield) {ierr=VecDestroy(&Hfield); CHKERRQ(ierr);}
                ...
                double **alphaList, double **betaList, int *matrixSize, PetscMPIInt rank)
{
   ...
   sprintf(resultsDir,"temp_%s/",projData->project_name);  // align this with OpenParEM2D.cpp and Hsolve.c
   ...
   PetscPrintf (PETSC_COMM_WORLD,"         matrix size: %zu\n",TtHeight+TzHeight);
   *matrixSize=TtHeight+TzHeight;
   ...
   if (projData->solution_shift_invert && use_initial_guess) {
      ...
      PetscPrintf(PETSC_COMM_WORLD,"            using initial guess for the dominant eigenvalue\n");
   }
   ...
   if (! projData->solution_shift_invert && use_initial_guess) {
      ...
      if (initial_guess_count > 0) {
         PetscPrintf(PETSC_COMM_WORLD,"            using initial guess for eigenvectors\n");
   ...
   if (projData->output_show_iterations) {
      ...
      PetscPrintf(PETSC_COMM_WORLD,"            iteration   # converged      alpha, dB/m           beta/ko             error\n");
      PetscPrintf(PETSC_COMM_WORLD,"            ----------------------------------------------------------------------------\n");
   ...
   if (projData->output_show_iterations) PetscPrintf(PETSC_COMM_WORLD,"            ----------------------------------------------------------------------------\n");

   // show results stats

   EPSGetIterationNumber(eps,&its);
   PetscPrintf(PETSC_COMM_WORLD,"            number of iterations: %ld\n",its);
