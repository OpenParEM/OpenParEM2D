...
void RectangularWaveguide::build_geo (Geo *geo)
{
   ...
   // create a curve
   Curve *newCurve=new Curve(1,"rectangluar_waveguide",true);
...
{
   if (active) *out << "Point(" << index << ") = {" << x << "," << y << "," << 0 << "," << grid << "};" << endl;
}

bool Point::is_equal (Point *test)
{
   if (double_compare (x,test->x,1e-14) && double_compare (y,test->y,1e-14)) return true;
   return false;
}
            ...
            if (pointList[j]->is_active()) {
               if (pointList[i]->is_equal(pointList[j])) {
                  ...
                  while (k < lineList.size()) {
                     if (lineList[k]->get_first()->get_index() == pointList[j]->get_index()) lineList[k]->get_first()->set_index(pointList[i]->get_index());
                     if (lineList[k]->get_second()->get_index() == pointList[j]->get_index()) lineList[k]->get_second()->set_index(pointList[i]->get_index());
                     k++;
                  }
...
bool RectangularWaveguide::write_geo (string indent, Control *control, Geo *geo)
{
   ...
   if (out.is_open()) {
      ...
      geo->write(&out);
      out << "Plane Surface(1) = {1};" << endl;
      out << "Physical Surface(\"" << material.get_value() << "\",1) = {1};" << endl;
...
bool RectangularWaveguide::write_modes_and_boundaries (string indent, Control *control)
{
   ...
   if (modes.is_open()) {
      ...
      modes << "   point=(" << 0 << "," << -height.get_dbl_value()/2 << ")" << endl;
      modes << "   point=(" << 0 << "," << +height.get_dbl_value()/2 << ")" << endl;
      ...
      if (boundary_material.compare("PEC") != 0) {
         ...
         modes << "   point=(" << -width.get_dbl_value()/2 << "," << +height.get_dbl_value()/2 << ")" << endl;
         modes << "   point=(" << +width.get_dbl_value()/2 << "," << +height.get_dbl_value()/2 << ")" << endl;
         modes << "   closed=false" << endl;
         modes << "EndPath" << endl;
...
bool RectangularWaveguide::write_proj(string indent)
{
   bool fail=false;

   stringstream ssProj;
   ssProj << name.get_value() << ".proj";
   ...
   if (proj.is_open()) {
      ...
      proj << "mesh.file                         " << name.get_value() << ".msh" << endl;
      proj << "mesh.order                        4" << endl;
      proj << "mode.definition.file              " << name.get_value() << "_modes.txt" << endl;
      ...
      proj << "solution.modes                    2" << endl;
      proj << "solution.temperature              20" << endl;
      proj << "solution.tolerance                1e-12" << endl;
      proj << "solution.iteration.limit          10000" << endl;
      proj << "solution.modes.buffer             5" << endl;
      proj << "solution.impedance.definition     PV" << endl;
      proj << "solution.impedance.calculation    modal" << endl;
...
void Strip::build_geo (Geo *geo, bool check_error)
{
   ...
   // lower
   Curve *newCurve=new Curve(1,"lower",true);
   if (use_symmetry.get_bool_value()) {
   ...
   // upper
   newCurve=new Curve(2,"upper",true);
   if (soldermask) {
      if (use_symmetry.get_bool_value()) {
   ...
   // soldermask
   newCurve=new Curve(3,"soldermask",false);
   if (soldermask) {
      newCurve->set_active();
      if (use_symmetry.get_bool_value()) {
         newCurve->push_line(geo->find_line(9),false);
...
bool Strip::write_geo (string indent, Control *control, Geo *geo)
{
   ...
   if (out.is_open()) {
      ...
      geo->write(&out);
      out << "Plane Surface(1) = {1};" << endl;
      out << "Physical Surface(\"" << lower_material.get_value() << "\",1) = {1};" << endl;
      out << "Plane Surface(2) = {2};" << endl;
      out << "Physical Surface(\"" << upper_material.get_value() << "\",2) = {2};" << endl;
      if (soldermask_thickness.is_loaded()) {
         out << "Plane Surface(3) = {3};" << endl;
         out << "Physical Surface(\"" << soldermask_material.get_value() << "\",3) = {3};" << endl;
      }
...
bool Strip::write_modes_and_boundaries (string indent, Control *control)
{
   ...
   if (modes.is_open()) {
      ...
      modes << "   point=(" << 0 << "," << 0 << ")" << endl;
      modes << "   point=(" << 0 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      modes << "   closed=false" << endl;
      modes << "EndPath" << endl;
      ...
      if (use_symmetry.get_bool_value()) {
         ...
         modes << "   point=(" << 0 << "," << 0 << ")" << endl;
         modes << "   point=(" << 0 << "," << lower_thickness.get_dbl_value()+upper_thickness.get_dbl_value() << ")" << endl;
         modes << "   closed=false" << endl;
         modes << "EndPath" << endl;
      ...
      modes << "   point=(" << -trace_width.get_dbl_value()/2+etch_offset << "," << lower_thickness.get_dbl_value()+trace_thickness.get_dbl_value() << ")" << endl;
      if (use_symmetry.get_bool_value()) modes << "   point=(" << 0 << "," << lower_thickness.get_dbl_value()+trace_thickness.get_dbl_value() << ")" << endl;
      else modes << "   point=(" << +trace_width.get_dbl_value()/2-etch_offset << "," << lower_thickness.get_dbl_value()+trace_thickness.get_dbl_value() << ")" << endl;
      modes << "   closed=false" << endl;
      modes << "EndPath" << endl;
      ...
      modes << "   point=(" << -trace_width.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      if (use_symmetry.get_bool_value()) modes << "   point=(" << 0 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      else modes << "   point=(" << +trace_width.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      modes << "   closed=false" << endl;
      modes << "EndPath" << endl;
      ...
      if (boundary_material.compare("PEC") != 0) {
         ...
         modes << "   point=(" << -left_side_gap.get_dbl_value()-trace_width.get_dbl_value()/2 << "," << 0 << ")" << endl;
         if (use_symmetry.get_bool_value()) modes << "   point=(" << 0 << "," << 0 << ")" << endl;
         else modes << "   point=(" << right_side_gap.get_dbl_value()+trace_width.get_dbl_value()/2 << "," << 0 << ")" << endl;
         modes << "   closed=false" << endl;
         modes << "EndPath" << endl;
      ...
      if (boundary_material.compare("PEC") != 0) {
         ...
         modes << "   point=(" << -left_side_gap.get_dbl_value()-trace_width.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value()+upper_thickness.get_dbl_value() << ")" << endl;
         if (use_symmetry.get_bool_value()) modes << "   point=(" << 0 << "," <<  lower_thickness.get_dbl_value()+upper_thickness.get_dbl_value() << ")" << endl;
         else modes << "   point=(" << right_side_gap.get_dbl_value()+trace_width.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value()+upper_thickness.get_dbl_value()  << ")" << endl;
         modes << "   closed=false" << endl;
         modes << "EndPath" << endl;
      ...
      if (boundary_material.compare("PEC") != 0 && !use_symmetry.get_bool_value()) {
         ...
         modes << "   point=(" << right_side_gap.get_dbl_value()+trace_width.get_dbl_value()/2 << "," << 0 << ")" << endl;
         modes << "   point=(" << right_side_gap.get_dbl_value()+trace_width.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value()+upper_thickness.get_dbl_value()  << ")" << endl;
         modes << "   closed=false" << endl;
         modes << "EndPath" << endl;
...
bool Strip::write_proj(string indent)
{
   bool fail=false;
   ...
   if (proj.is_open()) {
      ...
      proj << "refinement.iteration.max          50" << endl;
      proj << "refinement.required.passes        3" << endl;
      proj << "refinement.tolerance              1e-3" << endl;
      proj << "frequency.plan.linear             9e9,10e9,1e9" << endl;
      proj << "solution.modes                    1" << endl;
      proj << "solution.temperature              20" << endl;
      proj << "solution.tolerance                1e-10" << endl;
      proj << "solution.iteration.limit          30000" << endl;
      proj << "solution.modes.buffer             0" << endl;
      proj << "solution.impedance.definition     PI" << endl;
      proj << "solution.impedance.calculation    modal" << endl;
      proj << "solution.shift.invert             true" << endl;
      proj << "solution.use.initial.guess        true" << endl;
      proj << "output.show.refining.mesh         false" << endl;
...
bool CoupledStrip::write_modes_and_boundaries (string indent, Control *control)
{
   ...
   if (modes.is_open()) {
      ...
      modes << "   point=(" << -trace_left_width.get_dbl_value()/2-trace_air_gap.get_dbl_value()/2 << "," << 0 << ")" << endl;
      modes << "   point=(" << -trace_left_width.get_dbl_value()/2-trace_air_gap.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      modes << "   closed=false" << endl;
      modes << "EndPath" << endl;
      ...
      modes << "   point=(" << trace_right_width.get_dbl_value()/2+trace_air_gap.get_dbl_value()/2 << "," << 0 << ")" << endl;
      modes << "   point=(" << trace_right_width.get_dbl_value()/2+trace_air_gap.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      modes << "   closed=false" << endl;
      modes << "EndPath" << endl;
      ...
      modes << "   point=(" << -trace_left_width.get_dbl_value()-trace_air_gap.get_dbl_value()/2+etch_offset << "," << lower_thickness.get_dbl_value()+trace_thickness.get_dbl_value() << ")" << endl;
      modes << "   point=(" << -trace_air_gap.get_dbl_value()/2-etch_offset << "," << lower_thickness.get_dbl_value()+trace_thickness.get_dbl_value() << ")" << endl;
      modes << "   closed=false" << endl;
      modes << "EndPath" << endl;
      ...
      modes << "   point=(" << trace_air_gap.get_dbl_value()/2+etch_offset << "," << lower_thickness.get_dbl_value()+trace_thickness.get_dbl_value() << ")" << endl;
      modes << "   point=(" << trace_right_width.get_dbl_value()+trace_air_gap.get_dbl_value()/2-etch_offset << "," << lower_thickness.get_dbl_value()+trace_thickness.get_dbl_value() << ")" << endl;
      modes << "   closed=false" << endl;
      modes << "EndPath" << endl;
      ...
      modes << "   point=(" << -trace_left_width.get_dbl_value()-trace_air_gap.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      modes << "   point=(" << -trace_air_gap.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      modes << "   closed=false" << endl;
      modes << "EndPath" << endl;
      ...
      modes << "   point=(" << trace_air_gap.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      modes << "   point=(" << trace_right_width.get_dbl_value()+trace_air_gap.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value() << ")" << endl;
      modes << "   closed=false" << endl;
      modes << "EndPath" << endl;
      ...
      if (boundary_material.compare("PEC") != 0) {
         ...
         modes << "Path" << endl;
         modes << "   name=ground_plane" << endl;
         modes << "   point=(" << -left_side_gap.get_dbl_value()-trace_left_width.get_dbl_value()-trace_air_gap.get_dbl_value()/2 << "," << 0 << ")" << endl;
         modes << "   point=(" << right_side_gap.get_dbl_value()+trace_right_width.get_dbl_value()+trace_air_gap.get_dbl_value()/2 << "," << 0 << ")" << endl;
         modes << "   closed=false" << endl;
         modes << "EndPath" << endl;
      ...
      if (boundary_material.compare("PEC") != 0) {
         ...
         modes << "Path" << endl;
         modes << "   name=upper_groundplane" << endl;
         modes << "   point=(" << -left_side_gap.get_dbl_value()-trace_left_width.get_dbl_value()-trace_air_gap.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value()+upper_thickness.get_dbl_value() << ")" << endl;
         modes << "   point=(" << right_side_gap.get_dbl_value()+trace_right_width.get_dbl_value()+trace_air_gap.get_dbl_value()/2 << "," << lower_thickness.get_dbl_value()+upper_thickness.get_dbl_value()  << ")" << endl;
         modes << "   closed=false" << endl;
         modes << "EndPath" << endl;
...
bool CoupledStrip::write_proj(string indent)
{
   bool fail=false;
   ...
   if (proj.is_open()) {
      ...
      proj << "solution.modes.buffer             0" << endl;
      proj << "solution.impedance.definition     PI" << endl;
      proj << "solution.impedance.calculation    line" << endl;
      proj << "solution.shift.invert             true" << endl;
      proj << "solution.use.initial.guess        true" << endl;
      proj << "output.show.refining.mesh         false" << endl;
      proj << "output.show.postprocessing        false" << endl;
...
bool StructureDatabase::write_proj()
{
   ...
   while (i < rectangularWaveguideList.size()) {
      if (rectangularWaveguideList[i]->get_name().get_value().compare(controlList[0]->get_build().get_value()) == 0) {
         if (rectangularWaveguideList[i]->write_proj(indent)) return true;
         return false;
      }
      i++;
   }
   ...
   while (i < stripList.size()) {
      if (stripList[i]->get_name().get_value().compare(controlList[0]->get_build().get_value()) == 0) {
         if (stripList[i]->write_proj(indent)) return true;
         return false;
      }
      i++;
   }
   ...
   while (i < coupledStripList.size()) {
      if (coupledStripList[i]->get_name().get_value().compare(controlList[0]->get_build().get_value()) == 0) {
         if (coupledStripList[i]->write_proj(indent)) return true;
         return false;
      }
      i++;
   }
...
void printHelp ()
{
   ...
   PetscPrintf(PETSC_COMM_WORLD,"       Builder is a utility program for OpenParEM2D to build geometries and write setups\n");
...
int main (int argc, char *argv[])
{
   ...
   // write the needed files to run OpenParEM2D
