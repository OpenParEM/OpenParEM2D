...
void TestCaseDatabase::show_evaluation(ostream *out)
{
   ...
   *out << "# name,status,frequency,mode,column,";
      ...
      while (getline(inFile,line)) {
         ...
         if (line.compare("") != 0) {
            ...
            if (! is_hashComment(line)) {
               ...
               if ((!is_field && count == 7) || (!is_field && count == 8) || (is_field && count == 9) || (is_field && count == 10)) {
...
complex<double> Result::get_impedance (double frequency_, long unsigned int mode_, long unsigned int column)
{
   if (fabs((frequency-frequency_)/frequency) > 1e-14) return complex<double>(DBL_MAX,DBL_MAX);

   if (modalImpedanceCalculation && mode_ <  impedance.size()) return impedance[mode_];         // modal
   if (mode_+column*modeCount < impedance.size()) return impedance[mode_+column*modeCount];     // line
   cout << "ASSERT: Result::get_impedance invalid mode and/or column" << endl;
   return complex<double>(DBL_MAX,DBL_MAX);
}
