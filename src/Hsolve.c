...
PetscErrorCode InsertSubMatrix (Mat *A, Mat *B, PetscInt rowOffset, PetscInt colOffset, int show_stats, int show_data)
{
   ...
   if (show_stats) {
      PetscPrintf(PETSC_COMM_WORLD,"InsertSubMatrix: rowOffset=%ld colOffset=%ld\n",rowOffset,colOffset);
...
int Hsetup (struct projectData *projData, Mat *Mt, Mat *Cz, Mat *Zt, Mat *Mz, Mat *Ct, PetscMPIInt rank)
{
   ...
   if (projData->output_show_postprocessing) PetscPrintf(PETSC_COMM_WORLD,"            setting up for H field calculation ...\n");

   // directory to hold results
   resultsDir=(char *) malloc ((5+strlen(projData->project_name)+1+1)*sizeof(char));
   sprintf (resultsDir,"temp_%s/",projData->project_name);  // align this with OpenParEM2D.cpp and eigensolve.c
   ...
   //PetscPrintf (PETSC_COMM_WORLD,"ZtHeight=%zu  ZtWidth=%zu  ZtSparseWidth=%zu\n",ZtHeight,ZtWidth,ZtSparseWidth);
   //PetscPrintf (PETSC_COMM_WORLD,"MzHeight=%zu  MzWidth=%zu  MzSparseWidth=%zu\n",MzHeight,MzWidth,MzSparseWidth);
   //PetscPrintf (PETSC_COMM_WORLD,"CtHeight=%zu  CtWidth=%zu  CtSparseWidth=%zu\n",CtHeight,CtWidth,CtSparseWidth);
   //PetscPrintf (PETSC_COMM_WORLD,"matrix size: Ht=%zu, Hz=%zu\n",MtHeight,MzHeight);
            ...
            PetscInt EtSize, PetscInt EzSize, Vec *Efield, PetscScalar *gamma, Vec *Hfield, PetscMPIInt rank)
{
   ...
   if (projData->output_show_postprocessing) PetscPrintf(PETSC_COMM_WORLD,"               solving for H field ...\n");
